...
public enum RedisKey {
	/**all id，保存2小时*/
	WAITING_IDS("waiting_ids_agent", 2 * 60 * 60),
	/**all id，保存2小时 */
	ALL_IDS("all_ids_agent", 2 * 60 * 60),
	/**all test Token，保存2小时 */
	ALL_TEST_TOKENS("all_test_tokens_agent", 20),
	/**all Token，保存1天 */
	ALL_TOKENS("all_tokens_agent", 1 * 60),
	/**all un_read_message，保存1天 */
	UN_READ_MESSAGE("un_read_message_agent", 24 * 60 * 60),
	/**socket_session_tokens，保存1天 */
	SOCKET_SESSION_TOKENS("socket_session_tokens", 24 * 60 * 60),
	/** Token，保存1天 */
	TOKEN_API("token_agent_", 24 * 60 * 60),
	/** 商家Token，保存1天 */
	TOKEN_API_STORE("token_agent_store_", 8 * 60 * 60),
	/** 忘记登录密码图片验证码，保存10分钟 */
	FORGET_LOGIN_PIC_CODE("agent_flpc_", 10 * 60),
	/** 图片验证码，保存10分钟 */
	PIC_CODE("agent_pic_code_", 10 * 60),

	/** 短信验证码，保存10分钟 */
	SMS_CODE("agent_sms_code_", 10 * 60),
	...
	/**地区前缀*/
	STORE_CODE_PREFIX("store_code_prefix_", 72*60*60);
